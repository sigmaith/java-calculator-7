...
class ConverterTest {
    @Test
    void 기능_테스트() {
        Converter calculator = new Converter("1,2,3");
        assertThat(calculator.getResult()).isEqualTo(6);
    }

    @Test
    void 지정되지_않은_구분자_예외_테스트() {
        assertThatThrownBy(() -> new Converter("1?2,3")).isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining(UNDEFINED_DELIMITER.getDesciption());
    }

    @Test
    void 정수_범위를_벗어난_경우_예외_테스트() {
        assertThatThrownBy(() -> new Converter("123123123123123123123123")).isInstanceOf(
